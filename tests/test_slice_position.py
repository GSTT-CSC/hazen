...
class TestSlicePosition(unittest.TestCase):
    # EG has edited this section slightly so data is kept at top
    SLICE_POS_DATA = pathlib.Path(TEST_DATA_DIR / 'slicepos')
    ROD_COORDS_DATA = (122.22222222222223, 83.0, 132.88888888888889, 180.33333333333334)
    SLICE_POSITION_OUTPUT_DATA = ['0.151', '0.0964',  '0.237',  '0.101',  '0.224',  '0.103',  '0.0873',  '0.0386',  '0.0458',
    ...
    def test_get_rods_coords(self):
        ...
        assert (lx, ly, rx, ry) == self.ROD_COORDS_DATA

    def test_slice_position(self):
        results = hazen_slice_position.main(self.test_files)

        assert results == self.SLICE_POSITION_OUTPUT_DATA
...
class CanonTestSlicePosition(TestSlicePosition):
    ...
    SLICE_POS_DATA = pathlib.Path(TEST_DATA_DIR / 'slicepos')
    ROD_COORDS_DATA = (123.0, 77.5, 130.88888888888889, 171.66666666666666)
    SLICE_POSITION_OUTPUT_DATA = ['1.49','1.35','1.29','1.12','0.928','0.885','0.729','0.557','0.531','0.635','0.0882','0.437','0.215','0.238','0.196','0.0415','0.117','0.131','0','0.194','0.361','0.0941','0.424','0.265','0.446','0.464','0.587','0.646','0.672','0.779','1.07','0.880','0.940','0.948','1.23','1.29','1.58','1.41','1.71','1.95']

    def setUp(self):
        self.test_files = [pydicom.read_file(str(i), force=True) for i in (self.SLICE_POS_DATA / 'canon').iterdir()]
        self.test_files.sort(key=lambda x: x.SliceLocation)
